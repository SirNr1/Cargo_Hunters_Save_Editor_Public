...
### *The Save Management & Companion Suite for Cargo Hunters*
...
* **🎒 Inventory & Equipment Management**:
  ...
  * **Several rows at once.** Ctrl+click adds a row, Shift+click takes a range, and a right-click *inside* the selection keeps it instead of throwing it away. Repair, duplicate, move and delete then apply to all of them: you are asked once where things should go, and they are placed one at a time, because each item takes cells from the next. Whatever did not fit is named at the end rather than aborting the rest, and offered to the mailbox — except anything with parts fitted, which the game delivers one piece at a time and which would arrive as a pile. Split, stack size, attachments and item info stay on one row and are greyed out: a single value for items with different ceilings is not a sensible question, and the keyboard shortcuts refuse them too rather than walking around the grey.
  * **Move Item...** takes an item to another container. Attachments come along, and an equipped item leaves its slot empty. The destination list names every container and how much is free, and **marks the ones that cannot take this item** instead of hiding them. The size it is asking about is not always the size the save records: an assembled weapon is stored at what the game drew it as — a DVS at 4x1 — while it really covers what its parts make of it, so moving one into a strip of four free cells looked fine and came back as mail. What is asked for is the size the weapon actually covers: its template plus what its parts add.
...
* **📍 Choose Where a New Item Goes**:
  ...
  * **A weapon is measured by what it covers, and it needs its parts.** The size in the save is a floor, not the answer — a rifle stored at 1x1 still covers its template's 2x1, and dropping a stack onto *that* is what the game answers with mail. What it does **not** do is claim room beyond what it covers: the editor used to hold a weapon's full maximum clear as well, and stopped in September 2026 after the game accepted thirteen items laid into exactly those cells across two tests. One warehouse tab got 15 empty cells back. A weapon spawned on its own also gets the parts it cannot exist without, like a slide or a receiver — without them the game hands it straight back as mail.
* **📦 Full Game Items Catalog**:
  ...
  * **Spawn a weapon fully assembled**, the way the game ships it: right-click → **Spawn assembled...** and pick a configuration. 53 of them, covering 35 weapons with one to seven parts each, taken from the game's own preset list — the loot tables that leave anything to chance are left out rather than rolled for you. 17 configurations add up to more than their weapon is allowed to grow to, and the editor says so before spawning instead of letting the game answer with mail. The room it looks for is the **finished** size — template plus the parts of that configuration — and not the maximum the template could ever reach: measured against the weapons the game itself assembles, none is taller than two cells or wider than five, while asking for the maximum meant up to 7x4, nine times the area for a rifle the game stores at 3x1. Confirmed in play: weapons placed in gaps of exactly their finished size stay where they are put, one of them filling the last four cells of a full tab.
