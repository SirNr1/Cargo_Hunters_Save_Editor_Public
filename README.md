...
* **🎒 Inventory & Equipment Management**:
  ...
  * **Move Item...** takes an item to another container. Attachments come along, and an equipped item leaves its slot empty. The destination list names every container and how much is free, and **marks the ones that cannot take this item** instead of hiding them. The size it is asking about is not always the size the item covers: an assembled weapon is stored at what the game drew it as — a DVS at 4x1 — while the game wants its full size clear before accepting it, so moving one into a strip of four free cells looked fine and came back as mail. What is asked for now is the largest of what it covers, its maximum where the template can grow, and its own size plus what its parts add.
